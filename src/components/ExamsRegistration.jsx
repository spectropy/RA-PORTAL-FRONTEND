...
export default function ExamRegistration({ schools = [] }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (lastDashIndex <= 0 || lastDashIndex === classSection.length - 1) {
      alert(
        'Invalid Class-Section format. Expected format: "CLASS-SECTION" (e.g., "GRADE-6-A" or "6-A")',
      );
      return;
    }
  ...
  const downloadPDF = async (exam) => {
    ...
    import("jspdf").then((jsPDF) => {
      import("jspdf-autotable").then(() => {
        ...
        doc.setFontSize(18);
        doc.text(
          `Exam Results - ${exam.exam_pattern.replace("_", " ")} (${exam.class} - ${exam.section})`,
          14,
          22,
        );
        doc.setFontSize(12);
        doc.text(
          `School: ${exam.school_id} | Program: ${exam.program}`,
          14,
          30,
        );
        ...
        doc.save(
          `Exam_Results_${examId}_${new Date().toISOString().split("T")[0]}.pdf`,
        );
      });
    });
  };
  ...
  return (
    ...
    >
      <h2 style={{ color: "#1e90ff", marginBottom: "30px" }}>
        {" "}
        Exam Registration
      </h2>
          ...
          >
            ...
            <div>
              ...
              {schoolData.classes && schoolData.classes.length > 0
                ? [
                    ...new Set(
                      schoolData.classes.map((c) => c.program).filter(Boolean),
                    ),
                  ].join(", ")
                : "-"}
            </div>
            <div>
              <strong>Area:</strong> {schoolData.school?.area || "-"}
            </div>
            <div>
              <strong>Total Classes:</strong> {schoolData.classes?.length || 0}
            </div>
          </div>
        </div>
      )}

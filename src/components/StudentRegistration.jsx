...
export default function StudentRegistration({ schools = [] }) {
  ...
  const handleDeleteStudent = async (student) => {
    if (!selectedSchool || !student.id) {
      setError(
        "Cannot delete this student because the database record ID is missing.",
      );
      return;
    }

    const confirmed = window.confirm(
      `Delete ${getStudentDisplayName(student)}? This cannot be undone.`,
    );
    if (!confirmed) return;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const classValue =
        lastDashIndex > 0
          ? selectedClassSection.substring(0, lastDashIndex).trim()
          : selectedClassSection;
      const sectionValue =
        lastDashIndex > 0
          ? selectedClassSection.substring(lastDashIndex + 1).trim()
          : "";
  ...
  const downloadSample = () => {
    ...
    link.setAttribute(
      "download",
      `student_upload_template_${selectedSchool || "school"}.csv`,
    );
      ...
      {selectedSchool && selectedAcademicYear && schoolData && (
        <>
          ...
          {(selectedClassSection || students.length > 0) && (
            <div style={{ marginTop: "16px" }}>
              <div
                ...
                style={{ marginBottom: "8px", paddingBottom: "4px" }}
              >
                <div
                ...
                {students.length > 0 && (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: "4px",
                      cursor:
                        deletingClassSection || fetchingStudents
                          ? "not-allowed"
                          : "pointer",
                      fontSize: "12px",
                      fontWeight: "600",
                  ...
                  <table className="ct-compact-table">
                    <thead>
                      <tr>
                        ...
                        <th className="text-left">Parent Email</th>
                        <th>Actions</th>
                      </tr>
                    </thead>
                    <tbody>
          ...
          >
            ...
            >
              <li>
                Download and use the sample template to ensure correct column
                names
              </li>
              <li>
                <strong>Important:</strong> Student's class and section are set
                by your dropdown selection above, NOT the 'CLASS' column in the
                file
              </li>
